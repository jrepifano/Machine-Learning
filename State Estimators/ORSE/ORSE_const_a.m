% 2.1 ORSE Filter Constant Acceleration
...
M = [1 1/dt;1/dt 2/(dt^2)]*R;           % State covariance (initial)                                    
lambda = 9;
...
for i = 1:runs
   ...
   for k = 1:length(t)
       ...
       M = (eye(2)-K*H)*M*(eye(2)-K*H)'+K*R*K';     % Calc post cov
       D = (eye(2)-K*H)*D;                  % update lag
       stdPos(i,k) = 3 * sqrt(M(1,1));      % Store position covariance
       stdVel(i,k) = 3 * sqrt(M(2,2));      % Store velocity covariance
...
for k = 1:length(t)
    sigPos(1,k) = 3*sqrt(M(1,1));
    sigVel(1,k) = 3*sqrt(M(2,2));
end
negSigPos = -1 * sigPos;
negSigVel = -1 * sigVel;
fprintf('The filter 3 sigma posiiton error for const accel is: %f \n',mean(sigPos));
